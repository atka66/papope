...
if (global.player_connected[slotId]) {
	if (global.player_joined[slotId]) {
		...
		if (instance_find(obj_ctrl_lobby, 0).cntdwn) {
			drawSpriteText(x - 40, y + 22, "CANCEL", true, spr_controller_o);
		} else {
			drawSpriteText(x - 40, y + 22, "LEAVE", true, spr_controller_o);
		}
	} else {
		if (!instance_find(obj_ctrl_lobby, 0).cntdwn) {
			draw_sprite(spr_player_summon, 0, x, y - 32);
			drawSpriteText(x - 40, y + 22, "JOIN", true, spr_controller_x);
		}
	}
	draw_sprite_ext(spr_controller, 0, x, y, 1, 1, 0, ctrl_col, 1.0);
} else {
	draw_sprite_ext(spr_controller, 0, x, y, 1, 1, 0, ctrl_col, 0.5);
	drawText(x, y + 16, "OFFLINE", 1, c_white, 0.5, 1, false);
}